...
def entropy(sig):
	...
	# return 0.5*np.log2(np.linalg.det(sig)) + (n*np.log2(2*np.pi))/2 + n/2
...
def calc_MI_sklearn_regression(x, y, n_neighbors=3):
	...
	# mi = knnie.kraskov_mi(np.atleast_2d(x),np.atleast_2d(y).T, k=n_neighbors)

	return float(mi.squeeze())
...
def calc_MI_scipy(x, y, bins):
	# mi = knnie.kraskov_mi(np.atleast_2d(x).T,np.atleast_2d(y).T, k=3)
	# mi = knnie.revised_mi(np.atleast_2d(x).T,np.atleast_2d(y).T, k=3)
...
def calc_MI(x, y, bins, H_X_given, n):
...
def get_mi(noise_factor=1, dim=5, n_samples=1000, bins=[3,5]):
	...
	mu_e = np.atleast_1d(np.random.rand(1)) * noise_factor
	mu_e = np.atleast_1d(np.zeros(1))
	sig_e = np.atleast_2d(np.random.rand(1)) * noise_factor 
	...
	# print('entropy analytical:', H_x, H_y, H_xy)
	...
	for i in range(x.shape[0]):
		...
		#		 legend += [f'I_xy_sklearn_regression_bins_{bin}']
		#	 else:
		#		 I_xy_sklearn_regression[bin_i] += calc_MI_sklearn_regression(x_tmp, y_tmp, n_neighbors=bin)

		# for bin_i, bin in enumerate(bins):
		#	 if i == 0:
		#		 I_xy_sklearn += [calc_MI_sklearn(x_tmp.squeeze(), y_tmp, bin)]
		#		 legend += [f'I_xy_sklearn_bins_{bin}']
		#	 else:
		#		 I_xy_sklearn[bin_i] += calc_MI_sklearn(x_tmp.squeeze(), y_tmp, bin)

		for bin_i, bin in enumerate(bins):
			if i == 0:
				I_xy_samples += [calc_MI(x_tmp.squeeze(), y_tmp, bin, H_X_given=H_x, n=x.shape[0])]
				legend += [f'I_xy_samples_bins_{bin}']
			else:
				I_xy_samples[bin_i] += calc_MI(x_tmp.squeeze(), y_tmp, bin, H_X_given=H_x, n=x.shape[0])

		I_xy_scipy += calc_MI_scipy(x_tmp.squeeze(), y_tmp, bins[0])
		if i == 0:
			#  legend += [f'I_xy_scipy_bins_{bins[0]}']
			 legend += [f'I_xy_knnie_bins_{bins[0]}']
	...
	I_xy_scipy /= x.shape[0]
	# I_xy_samples /= x.shape[0]
	
	return np.array([*I_xy_samples, I_xy_scipy, I]), legend
...
if __name__ == '__main__':
	...
	# plot_samples(dim=140, n_samples=np.arange(15, 250, 5), bins=[3, 5, 10], noise_factor=0, n_runs=10)
	plot_samples(dim=50, n_samples=np.arange(10, 250, 5), bins=[3, 5], noise_factor=0, n_runs=10)
